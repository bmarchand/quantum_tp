...
type(ham_data) # ham_data is a list of 54 dictionnaries 
...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    # Hint: gates are regular python objects, they can be put in a list.  "Je comprends pas où ca va nous servir"
...
def phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10):
    ...
    """ Formula of phi_k seen in the course can't be applied to k= nBits. Hence for k= nBits, we impose that phi_k= 0, 
       whereas for k < nBits we use the formula of the course. """ 
    ...
    if trotterization== True:
        ...
        for k in range(nBits, 0, -1):
            ...
            if k==nBits :
                ...
                q = prog.qalloc(3)
                prog.apply(H,q[0])
                for i in range(2**(k-1)):      # Applying unitary operator U 2**(k-1) time.
                    prog.apply(trotter_ham_simulation(coeffs, dt, trotter_number, shift).ctrl(),q)
                ...
                max_state=0
                if result[0].probability < result[1].probability:    #Finding the state with maximum probability
                    max_state=1
        ...
        for k in range(nBits, 0, -1):
            ...
            if k==nBits :
                ...
                prog.apply(H,q[0])
                for i in range(2**(k-1)):    # Applying unitary operator U 2**(k-1) time. 
                    prog.apply(perfect_ham_simulation(coeffs, dt, shift).ctrl(),q)
                ...
                bits[k] = max_state
    phi= 0        
    for i in range(1,nBits+1):  #Determining phi using bits
...
for coeffs in ham_data:
    phi_perfect = phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10)
    phi_trotter_4 = phase(coeffs, trotterization=True, trotter_number=4, shift=-E_min, nBits = 10)
    phi_trotter_10 = phase(coeffs, trotterization=True, trotter_number=10, shift=-E_min, nBits = 10)
