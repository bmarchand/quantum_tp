...
q = prog.qalloc(2) # Allocation of a register of 2 qubits called q
...
prog.apply(ham_x(0.3).ctrl(), q) # The second instuction is the application of the ham_X gate on q
...
qpu = get_default_qpu() # Deployment of a numerical simulator

job = circ.to_job() # We want to compute the amplitude/probability of all possible states.
...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    for k in range(p):
        ...
        qroutine.apply(U_II(ham_coeffs["I_coeff"]*dt/p), 0, 1)
        qroutine.apply(U_II(shift*dt/p), 0, 1)
...
def phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10):
    ...
    for k in range(nBits, 0, -1):
        ...
        for l in range(k+1, nBits+1):
            phi_k = phi_k + 2*np.pi*bits[l]/2**(l-k+1)
