...
# We start by creating the gate ham_X, and constructing the circuit
...
# Now we can simulate the previously created circuit.
...
for sample in result:
    print(f"State {sample.state}: probability {sample.probability}, amplitude {sample.amplitude}")
...
def phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10):
    ...
    for k in range(nBits, 0, -1):
        ...
        # Initialisation of phi state
        prog.apply(X, q[1])
        
        # Application of the quantum circuit
        prog.apply(H, q[0])
...
plt.plot(Rs, vals_perfect, label="perfect", c='black')
plt.scatter(Rs, vals_trotter_4, label="p=4")
plt.scatter(Rs, vals_trotter_10, label="p=10")
plt.legend()
...
plt.scatter(Rs, (np.array(vals_perfect) - np.array(vals_trotter_4)) / np.array(vals_perfect) * 100, label="p=4")
plt.scatter(Rs, (np.array(vals_perfect) - np.array(vals_trotter_10)) / np.array(vals_perfect) * 100, label="p=10")
plt.plot(Rs, np.zeros((len(Rs),)), c='black')
plt.title('Relative deviation (%)')
plt.legend()


# On peut voir qu'on retrouve la tendance de la solution exacte, avec une meilleure précision lorsque p est plus grand.
