...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    qroutine = QRoutine()
    coeff_gates = [
        {'coeff': ham_coeffs['I_coeff']+shift, 'gate': U_II},
        {'coeff': ham_coeffs['Z0_coeff'], 'gate': U_ZI},
        {'coeff': ham_coeffs['Z1_coeff'], 'gate': U_IZ},
        {'coeff': ham_coeffs['Z0Z1_coeff'], 'gate': U_ZZ},
        {'coeff': ham_coeffs['X0X1_coeff'], 'gate': U_XX},
        {'coeff': ham_coeffs['Y0Y1_coeff'], 'gate': U_YY}
    ]
    for iterate in range(p):
        for coeff_gate in coeff_gates:
            qroutine.apply(coeff_gate['gate'](coeff_gate['coeff']*dt/p), q)
...
def phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10):
    ...
    for k in range(nBits, 0, -1):
        ...
        for l in range(k+1, nBits+1):
            phik += 2 * np.pi * bits[l]/(2**(l-k+1))
        
        # CIRCUIT CREATION
        # Put your implementation here. Paste here the QRoutines
        if trotterization:
            ham_simulation = trotter_ham_simulation(coeffs, dt * 2**(k-1), trotter_number, shift)
        else:
            ham_simulation = perfect_ham_simulation(coeffs, dt * 2**(k-1), shift)
        ...
        prog.apply(ham_simulation.ctrl(), q)
        
        prog.apply(PH(phik), q[0])
        prog.apply(H, q[0])
...
for coeffs in ham_data:
    ...
    E = phi_perfect / dt
    E_trotter_4 = phi_trotter_4 / dt
    E_trotter_10 = phi_trotter_10 / dt
