...
# displaying the states, amplitudes and probabilities
qpu = get_default_qpu()
job = circ.to_job()
...
if check:
    prog = Program()
    q = prog.qalloc(3)
    prog.apply(U_ZZ(3.), q[1:])
    prog.apply(U_ZI(3.), q[1:])
    prog.apply(U_IZ(3.), q[1:])
    prog.apply(U_XX(3.), q[1:])
    prog.apply(U_YY(3.), q[1:])
    circ = prog.to_circ()
...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    """
    qroutine = QRoutine()
    subroutines = [U_II,U_ZI,U_IZ,U_ZZ,U_XX,U_YY]
    keylist=["I_coeff","Z0_coeff","Z1_coeff","Z0Z1_coeff","X0X1_coeff","Y0Y1_coeff"]
    for i in range(p):
        qroutine.apply(U_II(dt*(shift+ham_coeffs["I_coeff"])/p),0,1)
        for j in range(1,6):
            qroutine.apply(subroutines[j](dt*ham_coeffs[keylist[j]]/p),0,1)
...
for coeffs in ham_data:
    phi_perfect = phase(coeffs,nBits=10)
