...
prog.apply(ham_x(0.3).ctrl(), [q[0],q[1]]) 
...
def U_YY(dt):
    ...
    yy_r.apply(S.dag(),1)
    yy_r.apply(CNOT, 0, 1)
    yy_r.apply(RZ(2*dt), 0) # difference of convention between implemented version and what we need.
    yy_r.apply(CNOT, 0, 1)
    yy_r.apply(S,1)
    return yy_r
...
def phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10):
    ...
    for k in range(nBits, 0, -1):
        ...
        maximum_proba = 0
        for sample in result:
            if sample.probability > maximum_proba:
                max_state = sample.state.int
                maximum_proba = sample.probability
...
for coeffs in ham_data:
    ...
    phi_trotter_100 = phase(coeffs, trotterization=True, trotter_number=100)
    # CONVERT PHASES BACK TO ENERGY
    E_perfect = phi_perfect*2*np.pi/dt
...
plt.plot(Rs, np.array(vals_perfect)*dt/2/np.pi, label="perfect")
plt.plot(Rs, np.array(vals_trotter_4)*dt/2/np.pi, label="p=4")
plt.plot(Rs, np.array(vals_trotter_10)*dt/2/np.pi, label="p=10")
plt.legend()
