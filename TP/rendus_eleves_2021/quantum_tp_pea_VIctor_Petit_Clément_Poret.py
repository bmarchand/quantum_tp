...
st = -1
print("amplitude               state   probability     corresponding number\n")

for sample in result:
    # print results
    print(sample.amplitude, sample.state, sample.probability, sample.state.int)
    st = sample.state
...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    list_routine = [[U_XX, U_YY, U_ZZ, U_IZ, U_ZI, U_II], ['X0X1_coeff', 'Y0Y1_coeff', 'Z0Z1_coeff', 'Z1_coeff', 'Z0_coeff', 'I_coeff']]
        
    for i in range(p):
        
        for j in range(5):
            qroutine.apply(list_routine[0][j](ham_coeffs[list_routine[1][j]] * dt/p), 0, 1)
            
        qroutine.apply(list_routine[0][5]((ham_coeffs[list_routine[1][5]]+shift) * dt/p), 0, 1)
...
def phase(coeffs, trotterization=False, trotter_number=4, shift=-E_min, nBits = 10):
    ...
    for k in range(nBits, 0, -1):
        ...
        phi_k = 2 * np.pi * sum([bits[i]/2**(i-k+1) for i in range(k+1, nBits+1)])
        ...
        # there are only two possible states, so the most likely is just the one with p>0.5
        if result[0].probability > 0.5:
            bits[k] = result[0].state.int
        else:
            bits[k] = 1 - result[0].state.int
        
                   
    # recompute phi
    phi = 2 * np.pi * sum([bits[l] / 2**(l) for l in range(1, nBits+1)])

    #phi = 2 * np.pi * sum([bits[i]/2**(i-k+1) for i in range(1, len(bits))])
...
plt.xlabel("Bond length [U.A. (likely Angström)]")
