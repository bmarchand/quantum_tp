...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    qroutine = QRoutine()
    u_ii = U_II((ham_coeffs["I_coeff"]+shift)*dt/p)
    u_zi = U_ZI(ham_coeffs["Z0_coeff"]*dt/p)
    u_iz = U_IZ(ham_coeffs["Z1_coeff"]*dt/p)
    u_zz = U_ZZ(ham_coeffs["Z0Z1_coeff"]*dt/p)
    u_xx = U_XX(ham_coeffs["X0X1_coeff"]*dt/p)
    u_yy = U_YY(ham_coeffs["Y0Y1_coeff"]*dt/p)
    
    
    for k in range(p):
        qroutine.apply(u_ii,0,1)
        qroutine.apply(u_zi,0,1)
        qroutine.apply(u_iz,0,1)
        qroutine.apply(u_zz,0,1)
        qroutine.apply(u_xx,0,1)
        qroutine.apply(u_yy,0,1)
...
def compute_phi_k(bits, nBits,k):
    phi_k = 2*pi*sum(bits[l]/2**(l-k+1) for l in range(k+1,nBits+1))
    return phi_k
