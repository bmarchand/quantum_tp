...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    for i in range(p):
        qroutine.apply(U_II(dt*(ham_coeffs[keys[0]]+shift)/p), 0, 1)
        qroutine.apply(U_ZI(dt*ham_coeffs[keys[1]]/p), 0, 1)
        qroutine.apply(U_IZ(dt*ham_coeffs[keys[2]]/p), 0, 1)
        qroutine.apply(U_ZZ(dt*ham_coeffs[keys[3]]/p), 0, 1)
        qroutine.apply(U_XX(dt*ham_coeffs[keys[4]]/p), 0, 1)
        qroutine.apply(U_YY(dt*ham_coeffs[keys[5]]/p), 0, 1)
