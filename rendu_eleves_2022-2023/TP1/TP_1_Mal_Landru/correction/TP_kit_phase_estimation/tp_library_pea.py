...
from qat.lang.AQASM import CNOT, RZ, RX, RY, S, I, PH
...
def U_I(dt,cara,p):
    ...
    g = np.diag([np.exp(-1j*dt*cara/p),np.exp(-1j*dt*cara/p),np.exp(-1j*dt*cara/p),np.exp(-1j*dt*cara/p)])
    u_r = AbstractGate("I",[float],arity=2,matrix_generator=g)
...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    for l in range(p):
            qroutine.apply(U_XX(dt*ham_coeffs['X0X1_coeff']/p), 0,1)
            qroutine.apply(U_YY(dt*ham_coeffs['Y0Y1_coeff']/p), 0,1)
            qroutine.apply(U_ZZ(dt*ham_coeffs['Z0Z1_coeff']/p), 0,1)
            qroutine.apply(U_IZ(dt*ham_coeffs['Z1_coeff']/p), 0,1)
            qroutine.apply(U_ZI(dt*ham_coeffs['Z0_coeff']/p), 0,1)
