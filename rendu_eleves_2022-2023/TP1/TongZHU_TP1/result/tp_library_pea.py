...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    qroutine = QRoutine()
    coeff0 = ham_coeffs['I_coeff']
    coeff1 = ham_coeffs['Z0_coeff']
    coeff2 = ham_coeffs['Z1_coeff']
    coeff3 = ham_coeffs['Z0Z1_coeff']
    coeff4 = ham_coeffs['X0X1_coeff']
    coeff5 = ham_coeffs['Y0Y1_coeff']

    # TODO write your code here
    for num in range(0,p):
      qroutine.apply(U_II((coeff0+shift)*dt/p), 0, 1)
      qroutine.apply(U_ZI(coeff1*dt/p), 0, 1)
      qroutine.apply(U_IZ(coeff2*dt/p), 0, 1)
      qroutine.apply(U_ZZ(coeff3*dt/p), 0, 1)
      qroutine.apply(U_XX(coeff4*dt/p), 0, 1)
      qroutine.apply(U_YY(coeff5*dt/p), 0, 1)
...
def compute_phi_k(bits, nBits,k):
    ...
    # TODO implementation here
    for l in range(nBits, k, -1):
      phi_k = 2*np.pi*bits[l]/2**(l-k+1) + phi_k
