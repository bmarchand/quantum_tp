...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    coeffs = ["I_coeff","Z0_coeff","Z1_coeff","Z0Z1_coeff","X0X1_coeff","Y0Y1_coeff"]
    for i in range(p): 
      qroutine.apply(U_XX(ham_coeffs[coeffs[4]]*dt/p),0,1)
      qroutine.apply(U_YY(ham_coeffs[coeffs[5]]*dt/p),0,1)
      qroutine.apply(U_ZZ(ham_coeffs[coeffs[3]]*dt/p),0,1)
      qroutine.apply(U_IZ(ham_coeffs[coeffs[2]]*dt/p),0,1)
      qroutine.apply(U_ZI(ham_coeffs[coeffs[1]]*dt/p),0,1)
      qroutine.apply(U_II((ham_coeffs[coeffs[0]]+shift)*dt/p),0,1)
