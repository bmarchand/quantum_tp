...
def U_XX(dt):
    ...
    # X x X = CNOT(0,1) . X x I . CNOT(0,1) , avec "x" produit de Kronecker et "." produit matriciel.
...
def U_YY(dt):
    ...
    #  Y x Y = S . Y x X . Sdag , avec "x" produit de Kronecker et "." produit matriciel.
...
def compute_phi_k(bits, nBits, k):
    phi_k = 0
    for j in range(k+1, nBits+1):
        if j in bits:
            phi_k += bits[j]/(2**(j-k+1))
