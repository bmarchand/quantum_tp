...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    keys = ["I_coeff", "Z0_coeff", "Z1_coeff", "Z0Z1_coeff", "Y0Y1_coeff", "X0X1_coeff"]
    g = [ham_coeffs[k] for k in keys]
    
    for _ in range(p):
        qroutine.apply(U_XX(g[5]*dt_r), 0, 1)
        qroutine.apply(U_YY(g[4]*dt_r), 0, 1)
        qroutine.apply(U_ZZ(g[3]*dt_r), 0, 1)
        qroutine.apply(U_IZ(g[2]*dt_r), 0, 1)
        qroutine.apply(U_ZI(g[1]*dt_r), 0, 1)
        qroutine.apply(U_II((g[0] + shift)*dt_r), 0, 1)
