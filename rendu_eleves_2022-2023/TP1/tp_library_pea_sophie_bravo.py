...
from qat.lang.AQASM import Program, H,  AbstractGate, QRoutine, CustomGate
from qat.lang.AQASM import CNOT, RZ, RX, RY, S, I, Z
...
def trotter_ham_simulation(ham_coeffs, dt, p, shift):
    ...
    for i in range(p):
      ...
      qroutine.apply(U_II(((shift+ham_coeffs["I_coeff"])*dt)/p), 0, 1)
    return qroutine
...
def compute_phi_k(bits, nBits,k):
    ...
    for l in range(k+1, nBits+1):
      phi_k = phi_k+2*pi*(bits[l]/2**(l-k+1))
